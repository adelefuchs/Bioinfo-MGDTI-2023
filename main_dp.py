...
def train_model(net, optimizer, fold_nums, epoch, features_list, node_type, type_emb, dg_seq, pt_seq, val, model_path):
    net.train()
...
def vaild_model(net, features_list, node_type, type_emb, dg_seq, pt_seq, val):
    #these are all tensors at this point
    print(f"Shape of features_list: {features_list[0].shape}")
    print(f"Shape of dg_seq: {dg_seq.shape}")
    print(f"Shape of pt_seq: {pt_seq.shape}")
    net.eval()
    with torch.no_grad():
    ...
    return auc, aupr, acc, val, pred

def save_results(true_values, predicted_values, file_path):
    import pandas as pd
...
def run():
        ...
        for fold_num in range(config.fold_nums):
            ...
            for epoch in range(config.num_epochs):
                train_model(net, optimizer, fold_num, epoch, features_list,
                            node_type, type_emb, dg_seq_train, pt_seq_train, train_set[:, 2], model_path)
                auc, aupr, acc, true_values, pred = vaild_model(net, features_list, node_type, type_emb,
                ...
                if auc > best_auc:
                    ...
                    best_predicted_values = pred
                    torch.save(net.state_dict(), model_path)
                    print(f"New best model saved at epoch {epoch} with AUC: {auc:.4f}")
            if best_true_values is not None and best_predicted_values is not None:
                save_results(best_true_values, best_predicted_values, f"predictions_fold{fold_num}.csv")
    ...
    # FINAL TESTING STEP (After cross-validation folds)
    print("Performing final test on the test dataset...")
    # Load the model from the last fold
    model_path = "best_model.pth"

    # Final test data (adjust if needed for your dataset)
    final_features_list, final_adjM, final_test_set, num_dgf, num_ptf  = dataset.load_final_test_data()  # Custom function to get final test data
    final_features_list = [mat2tensor(features).to(device) for features in final_features_list]
    
    # Create new all_dg_node and all_pt_node based on the new data
    final_num_dg = num_dgf
    final_num_pt = num_ptf
    print("FINAL ", final_num_dg, final_num_pt)
    final_all_dg_node = [i for i in range(final_num_dg)]
    final_all_pt_node = [i for i in range(final_num_dg, final_num_dg + final_num_pt)]

    g = g.to(device)
    # Process for final test set
    final_dg_seq = get_seqs(g, final_all_dg_node)
    final_pt_seq = get_seqs(g, final_all_pt_node)

    final_dg_index_test = final_test_set[:, 0]
    final_pt_index_test = final_test_set[:, 1]
    # Adjust protein indices by adding num_dg to match the earlier index shift
    final_pt_index_test_adjusted = final_pt_index_test - final_num_dg
    ...
    final_auc, final_aupr, final_acc, final_true_values, final_pred = vaild_model(net, features_list, node_type, type_emb, 
                                                                                  final_dg_seq[final_dg_index_test], final_pt_seq[final_pt_index_test_adjusted], final_test_set[:, 2])

    print(f"Final Test Results - AUC: {final_auc} AUPR: {final_aupr} Accuracy: {final_acc}")

    # Save final test results
    save_results(final_true_values, final_pred, "final_test_predictions.csv")
