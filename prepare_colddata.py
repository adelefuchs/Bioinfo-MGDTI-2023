...
class Prepare:
    ...
    def prepare_fold(self, dg_pt, node_list, node_type, delete_rate):
        ...
        negtive_index = np.random.choice(
            np.arange(len(whole_negetive_index)), size=positive_num, replace=False
        )
    ...
    def build_fold_cold_drug(
        self, fold_num, cold_drug_list, train_drug_list, delete_rate
    ):
        ...
        fold_train_index = []
        fold_test_index, fold_train_index_ = self.prepare_fold(
            dg_pt, cold_drug_list, 0, delete_rate
        )
        ...
        np.save(
            os.path.join(path, f"fold_{fold_num}_train_index.npy"), fold_train_index
        )
    ...
    def build_cold_drug(self, delete_rate):
        ...
        for fold_num in range(fold_nums):
            ...
            self.build_fold_cold_drug(
                fold_num, cold_drug_list, train_drug_list, delete_rate
            )
            cnt += split_len

    def build_fold_cold_protein(
        self, fold_num, cold_protein_list, train_protein_list, delete_rate
    ):
        ...
        fold_train_index = []
        fold_test_index, fold_train_index_ = self.prepare_fold(
            dg_pt, cold_protein_list, 1, delete_rate
        )
        fold_train_index.extend(fold_train_index_)
    ...
    def build_cold_protein(self, delete_rate):
        ...
        for fold_num in range(fold_nums):
            if fold_num < fold_nums - 1:
                ...
                train_protein_list = (
                    protein_list[0:cnt] + protein_list[cnt + split_len :]
                )
            else:
                cold_protein_list = protein_list[cnt:]
                train_protein_list = protein_list[:cnt]
            self.build_fold_cold_protein(
                fold_num, cold_protein_list, train_protein_list, delete_rate
            )
            cnt += split_len